...
func main() {
	log.Println("==================================================")
	log.Println("🧹 Cleaning up duplicate corrupted '?' groups")
	log.Println("==================================================")
	...
	// 2. Fetch all groups with '?'
	var badGroups []models.ProjectGroup
	db.Preload("Members").Where("project_name_th LIKE '%?%'").Find(&badGroups)
	fmt.Printf("Found %d corrupted groups to merge and clean.\n", len(badGroups))

	mergedCount := 0
	for _, badG := range badGroups {
		// Find matching clean group
		var cleanName string
		for _, m := range badG.Members {
		...
		if cleanName != "" {
			var cleanGroup models.ProjectGroup
			if err := db.Where("project_name_th = ?", cleanName).First(&cleanGroup).Error; err == nil {
				// Re-point submissions from bad group to clean group if not exists
				db.Model(&models.Submission{}).Where("group_id = ?", badG.ID).Update("group_id", cleanGroup.ID)
				db.Model(&models.PresentationBooking{}).Where("group_id = ?", badG.ID).Update("group_id", cleanGroup.ID)
				
				// Delete bad group members & bad group
				db.Where("group_id = ?", badG.ID).Delete(&models.GroupMember{})
				db.Delete(&badG)
				fmt.Printf("✅ Merged & Removed bad group [%s] -> [%s] '%s'\n", badG.ID, cleanGroup.ID, cleanName)
				mergedCount++
			} else {
				// Simply update bad group's name
				badG.ProjectNameTH = cleanName
				db.Save(&badG)
				fmt.Printf("✅ Renamed bad group [%s] to '%s'\n", badG.ID, cleanName)
				mergedCount++
			}
		} else {
			// Just delete empty orphaned corrupted group
			db.Where("group_id = ?", badG.ID).Delete(&models.GroupMember{})
			db.Delete(&badG)
		}
	}

	// Delete any remaining groups that still have '?'
	db.Where("project_name_th LIKE '%?%'").Delete(&models.ProjectGroup{})

	log.Printf("🎉 Successfully cleaned up %d corrupted groups!\n", mergedCount)
}
